...
`daspanel/load-balancer-caddy` as base image. 
...
``` sh
docker-compose run daspanel-load-balancer-caddy /bin/sh
```

where `daspanel-load-balancer-caddy` must be one of the 
...
``` sh
docker exec -ti load-balancer-caddy-load-balancer-caddy_1 /bin/sh
```
